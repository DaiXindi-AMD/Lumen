...
| **AITER** (submodule) | `~/Lumen/third_party/aiter` | ZhangDanyang-AMD/aiter (`lumen/qwen3-30b-a3b` branch) | Pinned kernel version for Lumen |
...
**跑 `tests/quantize/` 或任何会碰到 Megatron 的测试时，还要带上 `MEGATRON_ROOT`**：

```bash
PYTHONPATH=$HOME/Megatron-LM:$PWD python -m pytest tests/quantize/ -q
```

漏掉它不会报缺少环境变量，而是 `lumen/models/megatron_patches.py` 抛
`ModuleNotFoundError: No module named 'megatron'`，看起来像是 FP8 测试自己坏了
（`test_fp8_param_wiring.py` 三个）。训练启动脚本已经设了这个变量，所以只有手动跑
pytest 时会踩到。
