...
| Qwen3-8B | **MXFP4** | *running* | | 1.406x | | | | |

**The step-time result reproduces.** MXFP4 lands at 1.525x and 1.436x against
1.524x and 1.435x from the 50-step mock matrix — closer than the 1.5% noise
floor, on a different corpus at 7x the length. FP8 stays flat and Qwen3-8B's arm
is now 0.996x, i.e. marginally *slower* than BF16; with §10's error bar that is
still "indistinguishable from BF16", but it removes any reading of §9 as an
artefact of short runs or mock data.

**The loss curves are now real, and MXFP4 tracks BF16.** Held-out validation
loss, MXFP4 minus BF16: +0.064 (Llama2-7B), +0.055 (Llama3.1-8B). FP8 sits
within ±0.006. Both gaps are small and in the expected direction — MXFP4 is the
coarsest arithmetic here — and no run produced a NaN or a skipped iteration.
...
6. **The step-time result holds on real data at 7x the length** (§12): 1.525x and
   1.436x over 350 C4 steps against 1.524x and 1.435x over 50 mock steps, and
   Qwen3-8B's FP8 arm comes out at 0.996x, so §9 is not an artefact of short runs
   or random tokens.
7. **This is still not a full accuracy result.** 0 NaN / 0 skipped everywhere, and
   §12's held-out validation loss puts MXFP4 within +0.064 of BF16 on real text —
