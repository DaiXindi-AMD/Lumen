...
| 对 BF16 同配置（8574 ms） | 慢 1.43x | **快 1.406x** | 到 S10 才追平 |
...
*图 1 · 25 级累积阶梯：绿=无损且越过噪声线，灰=落在 ±1% 噪声内，红=变慢（只有 A6），橙=换显存（A18），蓝=只动尾延迟（A24）。三个大台阶 A2 / A5 / A11 全在 backward 算子链上。紫色虚线是同配置的 BF16（8574 ms）——前 10 级都还在它上面，见 3.1。*
...
## 三、对外参照：BF16、TE，以及和历史报告的关系

### 3.1 BF16 同配置：S24 快 1.406x，但要到 S10 才追平

BF16 在**完全相同的 recipe** 下测过（seq 8192 · MBS 2 · GBS 128 · 8×MI350X · Megatron · 同一个 mock 语料生成器，只是 50 步的 `t50` 而非 60 步的 `t60`），数字在 `mxfp4_precision_benchmark_report.md` 第 4 节，原始日志是 `examples/qwen3/results/lumen_qwen3_8b_bf16.log`：

| arm | 单步中位数 | vs BF16 | 峰值显存 |
| --- | --- | --- | --- |
| **BF16** | **8573.6 ms** | 1.000x | 0.6539 |
| S0 剥光的 MXFP4 | 12266.1 ms | **0.70x（慢 1.43x）** | 0.6179 |
| S10 | 8555.6 ms | 1.002x | 0.6179 |
| S24 = HEAD | **6096.6 ms** | **1.406x** | 0.6388 |

三件事值得单独拎出来：

**剥光的 MXFP4 比 BF16 慢 43%。**MXFP4 的 GEMM 理论吞吐是 BF16 的四倍，但 S0 慢 1.43x——这条路线的收益完全来自本报告这 24 项工程，不来自数据格式本身。

**要 10 项优化才追平 BF16。**S10（8555.6 ms）是第一个低于 BF16 的 rung，只领先 0.2%。也就是说 A1–A10 全部加起来只够抵消量化本身的开销，真正的净收益从 A11 才开始。

**MXFP4 在这个 recipe 下显存还更省**（HBM 占比 0.6388 vs 0.6539）。这与 `mxfp4_optimization_report.md` 记的"多 5.6 GB（+37%）"方向相反，因为那份是 FSDP2 / seq 2048 且开着跨 micro-batch 权重缓存，本实验是 Megatron 且 A4 在这条路径上没有兑现（4.3）。

### 3.2 S24 独立复现了已发布的 MXFP4 数字

同一份精度报告第 4 节的 Qwen3-8B MXFP4 arm 是 **6097.2 ms / 显存 0.6387**；本阶梯的 S24 是 **6096.6 ms / 0.6388**。两者相差 **0.6 ms（0.01%）**，显存差 0.0001。

两次测量的执行路径不同——精度报告跑的是常规 launcher，S24 跑的是 24 个消融开关全部显式设为"开"的 launcher——却落在同一个点上。这同时证明了两件事：**开关的默认值确实等于 HEAD 行为**（`tests/ops/test_ablation_ladder_arms.py` 断言的那条），以及**阶梯的测量口径与已发布数字可比**。两边的 warmup 口径也不同（精度报告丢前 10 步，本报告丢前 15 步），BF16 那条日志两种口径分别是 8573.6 和 8570.7 ms，差 0.03%，所以口径差异在这里可以忽略。

### 3.3 TransformerEngine：本实验没测，但对标做过，且已反超

TE 的对标全部在 `mxfp4_training_report.md`，不在本报告的范围内（那批都是 **GBS 32**，绝对 ms 与这里不可直接比）：

| 出处 | Lumen | TE | 结论 |
| --- | --- | --- | --- |
| §4.6 各自默认配置 | 2007.2 ms | 1425.3 ms | TE 快 1.41x，但 Lumen 收敛好 0.069 nats |
| §4.7 对齐 recipe（arm D） | 2007 ms | 1555.3 ms | TE 快 1.29x |
| §5.6 补完 tuned 表之后 | 1943.2 ms | 1556.1 ms | TE 快 1.25x |
| §5.13 `--lumen-linear` 转默认（8/10） | **1429.0 ms** | 1528.5 ms | **Lumen 反超 6.5%** |

§5.5 把 1.29x 逐类拆过（量化融合 +142 ms、Hadamard、RMSNorm、通信），§4.6.2 记了 ROCm 上 TE 的两个构建坑。当前口径见 `mxfp4_feature_parity_plan.md`：1429.0 对 TE 1528.5，收敛好 0.069 nats，两边零 NaN。

本报告的 24 项里有相当一部分正是这段反超的组成部分，但**阶梯本身不含 TE arm**，所以不要拿 S24 的 6096.6 去和上表的任何数字相减。

### 3.4 其余绝对值为什么对不上

除 3.1／3.2 那两个同配置的锚点之外，本实验**不复现历史报告里的数字**，这是设计使然，不是回归：
...
所以规则是：**recipe 完全一致时才比绝对 ms**（3.1／3.2 那两个锚点就是这么用的），其余情况只比相对降幅。另外，凡是本报告与历史 A/B 在**同一代码路径上**都测过的项（A4 / A17 / A24），两边可以互相印证——见 4.3 与 4.4。
...
图 1 里 BF16 那条参照线的数值写在 `plot_ladder.py` 的 `BF16_MS` 常量上，出处是 `mxfp4_precision_benchmark_report.md` 第 4 节，原始日志 `examples/qwen3/results/lumen_qwen3_8b_bf16.log`（未纳入本次提交，它属于那份报告）。
...
5. 3.1 引的 BF16 是 50 步、eval 间隔 5 的 run，本阶梯是 60 步、间隔 6。两种 warmup 口径下它只差 0.03%，所以现在这么引是安全的；但若要把 1.406x 写进对外材料，顺手用 `t60` 重跑一次 BF16 对齐口径更干净。
