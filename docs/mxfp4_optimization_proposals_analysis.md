...
**0.6B 规模 (Qwen3-0.6B, 8×MI350X, C4)**：

| 配置 | 中位步时 | 每 micro-batch 时间 |
|------|---------|-------------------|
| GA=1, 100 步 | 390.4 ms | 390.4 ms |
| GA=4, 100 步（缓存生效） | 1617.5 ms | 404.4 ms |
| GA=4 理想值（无缓存=4×GA1） | 1561.6 ms | 390.4 ms |

GA=4 的 per-micro-batch 时间与 GA=1 接近（+3.6%），差异来自 GA 模式的梯度累积内存压力，不是缓存开销。
...
GA=1 时缓存不带来额外节省（每步仍量化一次），性能持平。

**缓存的主要收益场景**：
- GA > 1：每 step 节省 `(GA-1)` 次权重量化（含 2D block scaling + padding）
- Gradient Checkpointing：backward 重算 forward 时跳过权重量化
- 两者叠加（GA=4 + grad ckpt）：每 step 节省 `2×GA - 1 = 7` 次权重量化
